...
class SpacedRepetitionManager {
  ...
  loadProgress() {
    ...
    if (saved) {
      ...
      this.progress = {};
      physicalFlashcards.forEach(card => {
  ...
  getDueCards() {
    const now = new Date();
    return physicalFlashcards.filter(card => {
...
const srManager = new SpacedRepetitionManager('physicalChemistryProgress');
...
function initSession() {
  ...
  if (totalDueDisplay) {
    totalDueDisplay.textContent = dueCards.length;
  }
